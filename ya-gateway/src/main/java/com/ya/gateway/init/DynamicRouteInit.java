...
import com.ya.gateway.entity.GatewayRouteList;
...
import static com.ya.gateway.common.GatewayConstant.*;
...
public class DynamicRouteInit {
	...
	public void initRoute() {
		try {
			...
			String content = configService.getConfig(CONFIG_DATA_ID_DYNAMIC_ROUTES, CONFIG_GROUP, CONFIG_TIMEOUT_MS);
			log.info("初始化网关路由开始");
			updateRoute(content);
			log.info("初始化网关路由完成");
			//开户监听，实现动态
			configService.addListener(CONFIG_DATA_ID_DYNAMIC_ROUTES, CONFIG_GROUP, new Listener() {
				@Override
